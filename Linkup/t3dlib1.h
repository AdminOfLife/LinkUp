...
// container structure for bitmaps .BMP file 代表一个.BMP文件
...
// the simple bitmap image 代表系统内存对象，和精灵一样可以被移动和绘制
...
int Draw_Bitmap32(BITMAP_FILE_PTR bitmap,DWORD *temp_buffer,int pos_x,int pos_y,int lpitch,int draw_mode,
				  LPRECT frame_size,int frame_x=0,int frame_y=0,UCHAR alpha=0,int edge=0);
