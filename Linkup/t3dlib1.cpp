...
FILE *fp_error                    = NULL; // 日志文件指针
char error_filename[80];                  // 日志文件名
...
extern HWND hwnd_app = NULL;
extern HINSTANCE hinstance_app = NULL; 
...
if (windowed)
   ...
   if (FAILED(lpdd->SetCooperativeLevel(hwnd_app,DDSCL_NORMAL)))
       return(0);
   ...
   if (FAILED(lpdd->SetCooperativeLevel(hwnd_app,
...
if (screen_windowed)
   ...
   if (FAILED(lpddclipperwin->SetHWnd(0, hwnd_app)))
       return(0);
...
if (FAILED(lpddclipper->SetHWnd(0,hwnd_app))) //Cliper在Windows模式下
{
	// release memory and return error
	free(region_data);
	return(NULL);
...
if (!screen_windowed)
   ...
   GetWindowRect(hwnd_app, &dest_rect);   
...
int Load_Bitmap_File(BITMAP_FILE_PTR bitmap, char *filename)
{
	...
	if ((file_handle = _open(filename,_O_RDONLY))==-1)
		return(0);

	// now load the bitmap file header
	_read(file_handle, &bitmap->bitmapfileheader,sizeof(BITMAPFILEHEADER));
	...
	// now load the bitmap file header
	_read(file_handle, &bitmap->bitmapinfoheader,sizeof(BITMAPINFOHEADER));

	// now load the color palette if there is one
	if (bitmap->bitmapinfoheader.biBitCount == 8)
	{
		_read(file_handle, &bitmap->palette,MAX_COLORS_PALETTE*sizeof(PALETTEENTRY));
	...
	if (bitmap->bitmapinfoheader.biBitCount==8 || bitmap->bitmapinfoheader.biBitCount==16) 
	{
		...
		// now read it in
		_read(file_handle,bitmap->buffer,bitmap->bitmapinfoheader.biSizeImage);
		...
		if (bitmap->bitmapinfoheader.biBitCount==24)
		{
			...
			_read(file_handle,temp_buffer,bitmap->bitmapinfoheader.biSizeImage);
...
int Load_Bitmap_File_32(BITMAP_FILE_PTR bitmap, char *filename)
{
	// this function opens a bitmap file and loads the data into bitmap
	...
	if (bitmap->bitmapinfoheader.biBitCount==8 || bitmap->bitmapinfoheader.biBitCount==16) 
	{
		...
		if (bitmap->bitmapinfoheader.biBitCount==24)
		{
			...
			// allocate final 16 bit storage buffer
			if (!(bitmap->buffer=(UCHAR *)malloc(4*bitmap->bitmapinfoheader.biWidth*bitmap->bitmapinfoheader.biHeight)))//这里改了
			{
				// close the file
				_close(file_handle);
...
} // end Load_Bitmap_File_32

//////////////////////////////////////////////



int Draw_Bitmap32(BITMAP_FILE_PTR bitmap,DWORD *temp_buffer,int pos_x,int pos_y,int lpitch,
				  int draw_mode,
				  LPRECT frame_size,int frame_x,int frame_y,
				  UCHAR alpha,
				  int edge
				  )
{
	//绘32位图像，pos_x和pos_y是目的坐标，当draw_mode为1是单元模式，
	//frame_size是单元格大小，frame_x和frame_y是单元坐标,
	//edge是图片边缘线，默认取0
	//注意此函数不具备错误检测

	int bitmap_width = bitmap->bitmapinfoheader.biWidth;
	int bitmap_height = bitmap->bitmapinfoheader.biHeight;
	int dest_height,dest_width;

	if(draw_mode == 1)
	{
		frame_x = edge + frame_x*(frame_size->right - frame_size->left + edge) ;
		frame_y = edge + frame_y*(frame_size->bottom - frame_size->top + edge) ;
		dest_height = frame_size->bottom - frame_size->top;
		dest_width = frame_size->right - frame_size->left;
	}
	else
	{
		dest_height = bitmap_height;
		dest_width = bitmap_width;
	}
	
	for (int index_y = 0; index_y < dest_height; index_y++)
	{
		for (int index_x = 0; index_x < dest_width; index_x++)
		{
			// get BGR values
			UCHAR blue  = (bitmap->buffer[(index_y+frame_y)*bitmap_width*4 + (index_x+frame_x)*4 + 0]),
				green = (bitmap->buffer[(index_y+frame_y)*bitmap_width*4 + (index_x+frame_x)*4 + 1]),
				red   = (bitmap->buffer[(index_y+frame_y)*bitmap_width*4 + (index_x+frame_x)*4 + 2]);

			// this builds a 32 bit color value in A.8.8.8 format (8-bit alpha mode)
			DWORD pixel = _RGB32BIT(alpha,red,green,blue);

			// write the pixel
			temp_buffer[pos_x+index_x + ((pos_y+index_y)*(lpitch >> 2))] = pixel;
...
int Window_Adjust(int win_width,int win_height,int side_x,int side_y)
{
	RECT window_rect = {0,0,win_width,win_height};

	if(!AdjustWindowRectEx(&window_rect,
		GetWindowStyle(hwnd_app),GetMenu(hwnd_app)!=NULL,GetWindowExStyle(hwnd_app)))
	{
		return 0;
	}

	if(!MoveWindow(hwnd_app,side_x,side_y,
